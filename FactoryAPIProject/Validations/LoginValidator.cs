...
namespace FactoryAPIProject.Validations
{
    public class LoginValidator : AbstractValidator<LoginModel>
    {
        public LoginValidator()
        {
            ...
            RuleFor(l => l.Password).NotNull().WithMessage("Password is not null").NotEmpty().WithMessage("Password is not empty").MinimumLength(3).WithMessage("Password must have at least three characters").Matches("^.{3,}$").WithMessage("Password must at least 3 character"); ;
        }
    }
}
